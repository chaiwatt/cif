...
$(document).on('click', '.pagination a', function (e) {
    ...
    var url = "/groups/salary-system/salary/calculation-list/calculation/search?page=" + page
    var paydayDetailId = $('#paydayDetailId').val();
    var data = {
        'searchInput': searchInput,
        'paydayDetailId': paydayDetailId
...
$(document).on('click', '#btn-import-employee-code', function (e) {
    ...
    var importIncomeDeductUrl = window.params.importIncomeDeductRoute
    ...
    RequestApi.postRequest(data, importIncomeDeductUrl, token).then(response => {
        // $('#table_container').html(response);
        $('#modal-income-deduct-assignment').modal('hide');
        Swal.fire(
            'สำเร็จ',
            'เพิ่มเงินได้เงินหักสำเร็จ',
            'success'
        );
    }).catch(error => {})
