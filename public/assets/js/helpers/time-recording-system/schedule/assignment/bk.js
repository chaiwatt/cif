...
function combineData(fileResultsArray, selectedEmployeeNos) {
    ...
    // console.log(specificResultsMap);
    // const firstElement = allData[0];
    // const dateInFirstElement = firstElement.Date;
    // const { firstDay, lastDay } = getFirstAndLastDayOfMonth(dateInFirstElement);
    ...
    for (const specificACNo of selectedEmployeeNos) {
        ...
        const dateInFirstElement = specificResults[0]['Date'];

        console.log(dateInFirstElement);
    ...
    for (const specificACNo of selectedEmployeeNos) {
        ...
        var firstDateForACNo = moment(getFirstNonEmptyDate(specificResults), 'DD/MM/YYYY');
        // console.log(firstDateForACNo);
        ...
        // console.log(specificACNo, selectedEmployeeNos[0])
        if (specificACNo === selectedEmployeeNos[0]) {
            // const isCrossDayShift = false;
            var isCrossDayShift = parseInt($('#schedule_type_id').val(), 10) === 2;

            // Convert startDate and endDate to moment objects for comparison
            const momentStartDate = moment(startDate, 'DD/MM/YYYY');
            const momentEndDate = moment(endDate, 'DD/MM/YYYY');

            // If isCrossDayShift is true, adjust the endDate to be one day after
            if (isCrossDayShift) {
                momentEndDate.add(1, 'day');
            }
            // console.log(finalResults);

            // Check if the startDate or endDate exists in finalResults
            const startDateExists = finalResults.some((row) => moment(row['Date'], 'DD/MM/YYYY').isSame(momentStartDate, 'day'));
            const endDateExists = finalResults.some((row) => moment(row['Date'], 'DD/MM/YYYY').isSame(momentEndDate, 'day'));
...
function getFirstNonEmptyDate(specificResults) {
    return specificResults.find(result => result.Date !== '')?.Date || null;
}
